...
RUN wget http://audeering.com/download/1318/opensmile-2.3.0.tar.gz && \
	tar -xf opensmile-2.3.0.tar.gz -C /usr

# download and extract OpenCV
RUN wget https://github.com/opencv/opencv/archive/2.4.13.3.zip && \
	unzip 2.4.13.3.zip -d /usr

# install OpenCV first
RUN mkdir -p /usr/opencv-2.4.13.3/release && cd /usr/opencv-2.4.13.3/release && \
	cmake -G "Unix Makefiles" -DCMAKE_CXX_COMPILER=/usr/bin/g++ CMAKE_C_COMPILER=/usr/bin/gcc -DCMAKE_BUILD_TYPE=RELEASE -DCMAKE_INSTALL_PREFIX=/usr -DWITH_TBB=ON -DBUILD_NEW_PYTHON_SUPPORT=ON -DWITH_V4L=ON -DINSTALL_C_EXAMPLES=ON -DINSTALL_PYTHON_EXAMPLES=ON -DBUILD_EXAMPLES=ON -DWITH_QT=ON -DWITH_OPENGL=ON -DBUILD_FAT_JAVA_LIB=ON -DINSTALL_TO_MANGLED_PATHS=ON -DINSTALL_CREATE_DISTRIB=ON -DINSTALL_TESTS=ON -DENABLE_FAST_MATH=ON -DWITH_IMAGEIO=ON -DBUILD_SHARED_LIBS=OFF -DWITH_GSTREAMER=ON .. && \
	# assuming most machines have 2 cores now
	make all -j2 && \
	make install

# install openSMILE
RUN cd /usr/opensmile-2.3.0/ && \
	./buildWithPortAudio.sh -o /usr/lib && \
	./buildStandalone.sh -o /usr/lib

# set working directory
WORKDIR /usr/opensmile-2.3.0/inst/bin
